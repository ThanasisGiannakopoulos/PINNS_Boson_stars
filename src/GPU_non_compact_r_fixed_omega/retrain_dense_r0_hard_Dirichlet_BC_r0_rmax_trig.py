...
from losses_hard_Dirichlet_BC_r0_rmax_trig import domain_loss, r0_loss, phi_monotonic_decrease_dom, alpha_monotonic_increase_dom
...
n = 2000  # Adjust this as necessary
# for grid to be more dense near r=0
sigma = 0.1

# Configuration and hyperparameters
out_dir = f"./models/hard_Dirichlet_BC_r0_rmax_trig_neurons{neurons}_h_layers{h_layers}_rmax{RMAX}_n{n}_sigma{sigma}/retrain/"
...
model.load_state_dict(torch.load(f"./models/hard_Dirichlet_BC_r0_rmax_trig_neurons{neurons}_h_layers{h_layers}_rmax{RMAX}_n{n}_sigma{sigma}/model_epoch500000.pth"))
...
epochs = 500000  # Total number of epochs
...
for epoch in range(epochs):
    ...
    alpha_mono_increase = alpha_monotonic_increase_dom(u, r, rmax)
    
    # Total loss
    loss = (w0 * loss_r0 + w1 * loss_dom + w3 * phi_mono_decrease + w4 * alpha_mono_increase)
    
    # save individual losses
    losses[0].append(loss_r0.item())
    losses[1].append(loss_dom.item())
    losses[2].append(phi_mono_decrease.item())
    losses[3].append(alpha_mono_increase.item())
    # save total loss
