...
from losses import domain_loss, r0_loss, rmax_loss, phi_monotonic_decrease_dom, alpha_monotonic_increase_dom
...
in_dir = f"./models/neurons{neurons}_h_layers{h_layers}_rmax{RMAX}_n{n}_sigma{sigma}/"
...
model_name = "model_epoch200000"
checkpoint_path = in_dir + model_name +".pth"  # or whichever checkpoint you want
...
# Unfreeze the last hidden layer in model.fch
# model.fch is a Sequential of Sequentials, so we access the last one like this:
last_hidden_layer = model.fch[-1][0]  # This gets the Linear layer inside the last Sequential block

for param in last_hidden_layer.parameters():
...
# mke out_dir to save retrained model
out_dir = in_dir + model_name + f"_last_layer_retrain_initial_lr{current_lr}_gamma{gamma}/"
