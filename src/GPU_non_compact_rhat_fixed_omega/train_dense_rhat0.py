...
out_dir = f"../../models/GPU_non_compact_rhat_fixed_omega/neurons{neurons}_h_layers{h_layers}_rmax{RMAX}_n{n}_sigma{sigma}/"
...
for epoch in range(epochs):
    ...
    # the loss at rhat0
    rhat0 = torch.zeros(1, requires_grad=True).to(device)
    u0 = model(rhat0)
    loss_r0 = r0_loss(u0, rhat0, phi0)
    ...
    rhat = torch.sinh(pts/sigma)/torch.sinh(torch.ones(1).to(device)/sigma)
    u = model(rhat)
    loss_dom = domain_loss(u, rhat, omega, m, RMAX)
    phi_mono_decrease = phi_monotonic_decrease_dom(u, rhat)
    alpha_mono_increase = alpha_monotonic_increase_dom(u, rhat)
    # loss at rmax = RMAX*rhat, for rhat=1
    rhat1 = torch.ones(1, requires_grad=True).to(device)
    umax = model(rhat1)
